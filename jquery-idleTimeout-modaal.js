 ...
 *               store.js from https://github.com/marcuswestin/store.js - v1.3.4+
 *
 * version 1.0.11
 **/
...
(function ($) {
  $.fn.idleTimeout = function (userRuntimeConfig) {
    ...
    var defaultConfig = {
      ...
      // idle settings
      idleTimeLimit: 60 * 30,      // 'No activity' time limit in seconds. 1200 = 20 Minutes
      ...
      dialogDisplayLimit: 60 * 3,       // Time to display the warning dialog before logout (and optional callback) in seconds. 180 = 3 Minutes
    ...
    openWarningDialog = function () {
      ...
      <p>` + currentConfig.dialogText + `</p>
      <p style='display:inline'>` + currentConfig.dialogTimeRemaining +
      `: <div style='display:inline' id='countdownDisplay'></div>
      </p></div>`;

      $('.idleConfirm').modaal({
        type: 'confirm',
        custom_class: 'modaalSessionConfirm',
        confirm_title: currentConfig.dialogTitle,
        accessible_title: currentConfig.dialogTitle,
        width: 400,
        after_open: function() {
          $('.modaal-ok').addClass('button--primary');
          $('.modaal-cancel').addClass('button');
        },
        confirm_content: dialogContent,
        confirm_button_text: currentConfig.dialogStayLoggedInButton,
        confirm_cancel_button_text: currentConfig.dialogLogOutNowButton,
      ...
      $(".idleConfirm").trigger("click"); // open the modaal
    ...
    destroyWarningDialog = function () {
      $(currentConfig.dialogIdentifier).modaal('close');
      ...
      elementContents.find('iframe,frame').each(function () {
        if ($(this).hasClass('jit-inspected') === false) {
          try {
            ...
            /* Unfortunately, if the 'includeIframes' function is manually
             * triggered multiple times in quick succession,
